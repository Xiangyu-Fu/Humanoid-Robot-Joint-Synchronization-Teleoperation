...
class TSIDWrapper:
    ''' Standard TSID formulation for a humanoid robot standing on its rectangular feet.
    ...
    def __init__(self, conf):
        ...
        contactRF.setKp(conf.kp_contact * np.array([1, 1, 1, 1, 1, 10]))
        ...
        contactLF = tsid.Contact6d("contact_lfoot", robot, conf.lf_frame_name, feet_contact_vertices,
                                   conf.contactNormal, conf.f_mu, conf.f_fMin, conf.f_fMax)
        contactLF.setKp(conf.kp_contact * np.array([1, 1, 1, 1, 1, 10]))
        contactLF.setKd(2.0 * np.sqrt(contactLF.Kp))
        ...
        self.leftFootTask.setKp(
            self.conf.kp_foot * np.array([1, 1, 1, 1, 1, 10]))
        ...
        self.LH = self.model.getFrameId(self.conf.lh_frame_name)  
        ...
        # left hand reference
        T_lh_w = self.robot.framePosition(data, self.LH)  # assuming 'data' is your data container
        self.lh_ref = create_sample(T_lh_w)  # assuming 'create_sample()' is a function in your code
        ...
        self.RH = self.model.getFrameId(self.conf.rh_frame_name)
    ...
    def setAmRefState(self, amRef):
        ref = create_sample(amRef)
        self.amTask.setReference(ref)
    ...
    def set_LF_pose_ref(self, pose, vel=None, acc=None):
        ...
        # print('\r lf_ref=', pose, end='')
        self.leftFootTask.setReference(self.lf_ref)

    def set_LF_pos_ref(self, pos, vel=None, acc=None):
        X = self.lf_ref.value(); X[:3] = pos
        V = self.lf_ref.derivative(); V[:3] = vel
        A = self.lf_ref.second_derivative(); A[:3] = acc
        update_sample(self.lf_ref, X, V, A)
        self.leftFootTask.setReference(self.lf_ref)

    # Hand tasks
    def add_motion_LH(self, pose, vel=None, acc=None):
        update_sample(self.lh_ref, pose, vel, acc)
        self.rightHandTask.setReference(self.lh_ref)
    
    def add_motion_RH(self, pose, vel=None, acc=None):
        update_sample(self.rh_ref, pose, vel, acc)
        self.leftHandTask.setReference(self.rh_ref)

    def set_RH_pos_ref(self, pos, vel=None, acc=None):
    ...
    def set_LH_pos_ref(self, pos, vel=None, acc=None):
    ...
    def remove_contact_RF(self, transition_time=0.0):
        if self.contact_RF_active:
            ...
            self.formulation.removeRigidContact(
                self.contactRF.name, transition_time) # BUG: poosible bug here
